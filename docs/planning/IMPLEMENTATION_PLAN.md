...
| 7. 完整能力与状态 | 当前内容已实现，Godot 验收待执行 | `AbilityDefinition` 保存发动类型、目标和效果；`PendingAbility` 携带来源与回响标记，队列负责发动与连锁；被动光环按当前状态求值。无需为现有规则单独建立同义类型。 |
| 8. 遭遇排程与轮次 | 已实现 | 商店保底、特殊回合、轮次推进和确定性验证均已存在。 |
| 9. 完整对局闭环 | 当前内容已实现，Godot 验收待执行 | `GameCoordinator` 协调创建、遭遇推进、战斗和结算；商店与事件分别通过应用用例执行。`MinimalPlaytest` 的 `Screen` 只控制界面呈现。 |
| 10. Godot 表现层 | 部分实现 | 选角、遭遇、商店、双棋盘和战斗结果已有试玩界面；对局/棋盘/遭遇刷新现读取 `MatchSnapshot`，操作交给应用用例。像素布局、拖拽预览、战斗表现方案、资源映射及集成/人工验收清单仍未完成。 |

### 当前执行队列

1. **第 7、9 步 Godot 验收**：用现有验证场景和试玩入口复核能力、对局结果与新快照；发现具体差异后修复。
2. **内容阶段 D → E 验收**：技能实例、通用战斗来源、技能合并升级及连续合并验证已接入；在 Godot 入口验收。
3. **内容阶段 F → G**：套装阈值与卡牌任务均通过能力解锁；任务实例进度、战斗胜利条件及战场能力生效已实现。待具体内容定义其他条件、非数值效果及正式套装/任务，再扩充其他内容。
...
- 完成 `AbilityDefinition` 的发动类型、目标和效果定义，并由 `PendingAbility` 携带单次执行的来源与回响信息。
- 主动能力与回响进入 FIFO 队列；被动光环按当前战斗状态求值，来源失效时立即停止贡献。
- 实现需求列出的强类型战斗事件。
- 执行前检查来源和主动能力魔法；通过回响来源标记截断连锁。
- 实现伤害、治疗、护甲即时效果。
